...
public class proxy : IHttpHandler,IRequiresSessionState
{
    ...
    string SSO = ConfigurationManager.AppSettings.Get("SSO");
    ...
    public bool ValidateToken(string token)
    {
        ...
        string introspectEndPoint = $"{SSO}/connect/introspect";
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Session["NameSSO"] != null)
        {
            string NameSSO = context.Session["NameSSO"].ToString();

            if (!ValidateToken(NameSSO))
            {
                string errorMsg = "This proxy does not support u";
        ...
        {
            string errorMsg = "This proxy does not support  this way";
            sendErrorResponse(context.Response, null, errorMsg, System.Net.HttpStatusCode.BadRequest);
            return;
        }
    }
